  ...
  const handleCreateApplicant = async () => {
    try {
      const response = await apiService.createApplicant({
        ...
        status: newApplicant.status as "pending" | "processing" | "approved" | "rejected",
      });
      ...
      setApplicants((prev) => [newApplicantWithDetails as Applicant, ...prev]);
  ...
  const handleUpdateStatus = async (applicantId: string, newStatus: string) => {
    try {
      const response = await apiService.updateApplicant(applicantId, {
        status: newStatus as "pending" | "processing" | "approved" | "rejected",
      });
      ...
      setApplicants((prev) =>
        prev.map((applicant) =>
          applicant.id === applicantId
            ? { ...applicant, status: newStatus as "pending" | "processing" | "approved" | "rejected" }
            : applicant
        )
      );
    } catch (error) {
